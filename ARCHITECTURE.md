...
- **`manual` SourceDriver** — OAuth 없음. 운영자가 UI에서 파일을 `raw/{tenant}/{source_id}/{content_hash}/{filename}`에 적재. 원격 `collect_source`/`Run now` 대신 **upload + ingest** API 사용.
...
- **Ingest** — `ingest_state=pending` 문서만 mini-batch manifest → Argo `pipeline-ingest-rag` (원격 Source `run`과 동일 후반). 삭제·compensation은 MVP 제외.
- **BFF** (admin + CSRF): `POST /api/pipeline/sources/{id}/upload` (multipart), `GET .../documents`, `POST .../ingest`. `driver=manual`만 upload/ingest 허용.
