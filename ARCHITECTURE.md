...
인프라 소유: **Garage·runtime PG(pgvector)** → agents-runtime, **Argo Workflows·Nebula** → path-graph [`deploy/k8s/`](deploy/k8s/) (설치·운영).
...
- **S3 (Garage)** — ingest artifact source of truth (raw·parsed·chunks·graph_context 등). **Wiki 본문**은 runtime PG `vfs_wiki_files`. PG(pgvector 포함)·Nebula는 파생 인덱스.
...
- **도메인** — `path_graph.admin.{projects,sources,runner}` (path-graph 패키지 내부). agents-runtime을 import 하지 않음.
- **Console 공개 API** — 외부 소비자(agents-runtime)는 **`path_graph.console`만** import. `path_graph.admin`은 path-graph 내부·Argo step 전용.
...
- **Ingest** — `ingest_state=pending` 문서만 mini-batch manifest → Argo `pipeline-ingest-rag`. **재 ingest·수정** 시 `compensate_document_index` 선행(구 chunk embedding·Nebula 삭제).
...
RAG ingest 이후 Graph(Nebula)·Wiki(PG VFS) 적재. Console MVP는 **`pipeline-graphrag` 단일** WF.
...
```json
{
  ...
  "rag": {
    "index_namespace": "path_graph_acme_product-docs",
...
- **확장 (Pool)** — tenant당 논리 `index_namespace` 1개 + PG `project_id` 필터. binding API는 동일; `index_namespace`만 `path_graph_{tenant_slug}`로 바뀐다.

### 테넌트 격리

- **`tenant`는 모든 가변 데이터의 1차 파티션 키**다. S3 prefix, PG 행, Nebula Space, Argo workflow parameter 어디에든 누락되면 안 된다.
- **PostgreSQL** (`path_graph` schema, runtime DB): 모든 테이블에 `tenant TEXT NOT NULL`. PK는 `(tenant, id)` 복합. **벡터**는 `chunks.embedding vector(1024)` — 별도 vector DB 없음. 검색은 `tenant` + `project_id` 필터 + HNSW(cosine). Phase 1부터 **RLS** (`tenant = current_setting('app.tenant')`) 적용.
- **NebulaGraph** (path-graph infra): **project당 Space 1개** — `index_namespace`와 동일 이름. 엣지 탐색은 Space 경계 밖으로 나가지 않는다.
- **Garage S3**: `raw|parsed|chunks|...` — [`§2.1`](#21-s3-garage). Wiki 본문은 S3에 두지 않는다.
...
2. runtime PG `path_graph.*` — 메타·상태·벡터(`chunks.embedding`)
3. Nebula — graph upsert (Graph/Wiki downstream만)
...
- **Overwrite 규칙**: 재시도 시 PG·Nebula는 동일 `chunk_id`로 upsert. PG embedding은 동일 `chunk_id`로 덮어쓴다. Nebula vertex id = 계약된 deterministic id.
- **보상(Compensation)**: 재 ingest·문서 수정 시 **선행** `compensate_document_index` → PG embedding clear → Nebula chunk·MENTIONS 삭제 → entity orphan prune → 재적재.
- **정보삭제(Purge)**: `ingest_state=purged` + tombstone `(tenant, project_id, content_hash)`. upload·upsert·manifest 차단.
- **IndexReconcile**: PG `chunks`를 truth로 Nebula 고아 주기 삭제(Cron `pipeline-reconcile-index`).
...
- Nebula **설치 매니페스트**를 `deploy/k8s/infra/` 밖(예: pipeline 패키지)에 두지 말 것.
...
| `chunks` | `(tenant, id)`, `project_id`, `document_id`, `chunk_index`, text, `text_tsv`, `embedding vector(1024)` |
...
| `wiki_pages` | `(tenant, project_id, slug)`, `community_id`, `batch_id`, `title` — 파이프라인 인덱스; 본문은 `vfs_wiki_files` |
...
| `indexed_rag` | pgvector upsert 완료 (`chunks.embedding`) | `mark_rag_indexed` |
...
### 2.3 pgvector (runtime PostgreSQL)

- **저장**: `path_graph.chunks.embedding vector(1024)` — point id = `chunk_id` (행 PK)
- **검색**: `PgMetaStore.search_vector` — `WHERE tenant AND project_id` + `ORDER BY embedding <=> query` (cosine)
- **인덱스**: HNSW `vector_cosine_ops` + partial `(tenant, project_id)` btree
- **dim 1024**, cosine — embedding은 **cluster 외부** OpenAI-compatible HTTP (`EMBEDDING_BASE_URL`, `EMBEDDING_MODEL`, `EMBEDDING_DIM`). path-graph는 TEI 등 구현체를 소유하지 않는다 (D4).
- **extension**: runtime Postgres **17** + `pgvector` (`pgvector/pgvector:pg17`). `CREATE EXTENSION vector` — agents-runtime [`postgres.yaml`](../agents-runtime/deploy/k8s/base/postgres.yaml)
- **binding** `index_namespace`: `path_graph_{tenant_slug}_{project_slug}` — Nebula space와 동일 논리명 (`ids.index_namespace`)

### 2.4 NebulaGraph (path-graph infra)

- Space: `path_graph_{tenant_slug}_{project_slug}` — `index_namespace`와 동일 (`ids.nebula_space_name`)
...
|---|---|
| [agents-runtime](../agents-runtime) | Garage, runtime PG(pgvector), `POST /v1/agents/invoke` |
| path-graph [`deploy/k8s/`](deploy/k8s/) | Argo Workflows controller, NebulaGraph (설치·운영) |
| [rhwp_batch](../rhwp_batch) | HWP/HWPX `to-json` 컨테이너 이미지 |

**로컬 개발 연결**: [`scripts/wire-dev.sh`](scripts/wire-dev.sh) — PG `:5432`, Envoy `:8084`, Nebula `:9669`, Garage `:3900`(profile s3). 포트 맵: [`scripts/wire-dev.env.example`](scripts/wire-dev.env.example). TEI(선택): `llm-serving/bge-m3-tei` → `:8085`. **Garage 브라우저 UI**(k8s dev): Filestash → [`deploy/SETUP.md`](deploy/SETUP.md#filestash-garage-s3-ui).
