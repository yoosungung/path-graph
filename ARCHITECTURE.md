...
```json
{
  ...
  "wiki": { "s3_prefix": "wiki/acme/550e8400-.../", "vfs_mount": "/wiki" }
}
```
...
- **Qdrant** (test_infra): **project당 collection 1개 (Silo MVP)** — `path_graph_{tenant_slug}_{project_slug}`. point payload에 `tenant`, `project_id` 필수. 검색은 에이전트 binding의 collection 1개 + `filter.project_id`. (과거 `stable_hash(chunk_id) % n` 샤딩 **폐기**.)
- **NebulaGraph** (test_infra): **project당 Space 1개** — Qdrant collection과 동일 이름. 엣지 탐색은 Space 경계 밖으로 나가지 않는다.
