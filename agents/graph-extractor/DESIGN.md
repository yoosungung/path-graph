...
`load`는 chunks.jsonl → 100청크 그룹 → 그룹별 char-batch 목록(`chunk_groups`). `extract`는 그룹을 순서대로, 그룹 안에서는 `max_concurrent_workers`만큼 병렬 LLM 호출 후 `merge_graph_parts`.

출력이 `max_completion_tokens`에 닿거나 LLM이 **빈 본문**을 주면 해당 배치 텍스트를 **반으로 쪼개 재귀 호출**한다. `min_split_chars` 이하이면 그 배치는 `{"entities":[],"edges":[]}`로 건너뛴다(전체 job 실패 방지).
...
#   name=graph-extractor, version=v8, runtime_pool=agent:compiled_graph
#   entrypoint=graph_extractor.agent:factory
./scripts/register-agent-bundles.sh graph-extractor v8
