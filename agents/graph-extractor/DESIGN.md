...
|------------------------------|--------|------|
| `chunks_per_group` | `100` | 청크 **100개** 단위 super-batch — 그룹 간 **순차** 처리 |
| `max_concurrent_workers` | `2` | super-batch 내 LLM char-batch **동시 2** |
| `max_batch_chars` | preset 계산 또는 `4000` | LLM 1회당 청크 텍스트 상한(문자) |
| `max_completion_tokens` | preset 계산 또는 `4096` | `llm.bind(max_tokens=…)` |

`load`는 chunks.jsonl → 100청크 그룹 → 그룹별 char-batch 목록(`chunk_groups`). 각 청크 텍스트는 LLM 호출 전 `text_sanitize.sanitize_chunk_text`로 정리한다(HWP LaTeX 잔재·줄바꿈·surrogate 제거).

`extract`는 그룹을 순서대로, 그룹 안에서는 `max_concurrent_workers`만큼 병렬 LLM 호출 후 `merge_graph_parts`. 출력은 `sanitize_graph_v1` + `ensure_json_utf8_safe`로 job JSON 직렬화 전 정리한다.

출력이 `max_completion_tokens`에 닿거나 LLM이 **빈 본문**을 주면 해당 배치를 `split_batch_text`로 쪼갠다 — 우선 `\n\n`(청크 경계), 다음 `\n`(행 경계), 최후에 문자 중간 분할. `min_split_chars` 이하이면 그 배치는 `{"entities":[],"edges":[]}`로 건너뛴다.
...
#   name=graph-extractor, version=v9, runtime_pool=agent:compiled_graph
#   entrypoint=graph_extractor.agent:factory
./scripts/register-agent-bundles.sh graph-extractor v9
