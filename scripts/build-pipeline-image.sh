...
if [[ "${PUSH:-}" == "1" ]]; then
  ...
  docker run --rm --platform "$PLATFORM" "$IMAGE" -c "import openpyxl, xlrd, pymupdf4llm; from unstructured.partition.docx import partition_docx; from unstructured.partition.pptx import partition_pptx; from unstructured.partition.xlsx import partition_xlsx; print('ok')"
  echo "Smoke OK: rhwp-batch v${RHWP_BATCH_VERSION} + native PDF/Office deps"
fi
