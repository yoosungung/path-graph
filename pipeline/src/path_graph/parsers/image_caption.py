"""Crop PDF image regions and fill image-block captions via Vision API (#280)."""
...
def enrich_image_block_captions(
    ...
    *,
    ...
    """Fill empty ``image`` captions via crop → Vision, preserving block order.

    Skips blocks that already have a caption or lack page/bbox metadata.
    Caption failures leave the block unchanged (adjacent-text caption may remain empty).
    """
    ...
    for block in blocks:
        ...
        try:
            ...
            caption = ocr.ocr_page_png(png, prompt=prompt).strip()
        except Exception:
            continue
        if caption:
