...
| `lifecycle/compensation.py` | re-ingest·purge 전 Qdrant/Nebula 정리. Nebula space가 없으면 chunk 삭제 skip(0) — graph 미적재·이미 drop된 프로젝트 purge idempotent |
...
project **내부** PG FTS + Qdrant vector 두 채널을 RRF로 병합한다. multi-project RRF(PG-4)는 agents-runtime `runtime_common.knowledge`가 담당.

| 모듈 | 역할 |
|------|------|
| `path_graph.meta.pg` | `chunks.text_tsv` GIN + `search_fts()` |
| `path_graph.rag.qdrant_store` | `search()` — cosine ANN |
