...
collect / ingest_web
  ...
  → parse (포맷별 native → content.json blocks[]; HWP=rhwp-batch)
  → chunk_from_blocks (CHUNK_MAX_CHARS; type-aware)
  → S3: parsed/{tenant}/{doc_id}/content.json (+ optional content.md), chunks/.../chunks.jsonl
...
**결정 (ROADMAP D3 개정)**: 포맷별 **native parser**가 직접 `content.json` `blocks[]`를 생성한다. 청킹은 항상 `blocks[]`만 본다. `content.md`는 optional debug. **`markitdown` / `md_heuristic` / md 필수 경로 없음** (출시 전 legacy 없음).

### 파이프라인

```
raw bytes
  → router(extension + PDF text/image ratio)
  → parser native structure
       - HWP/HWPX: rhwp-batch JSON blocks
       - Office (.docx/.pptx/.xlsx/.xls): Unstructured lightweight typed elements
...
`rhwp-batch` JSON `blocks[]` 유지 — `extractor=rhwp_batch`.

### Chunking

- `paragraph` / `list_item`: 논리 경계 우선, 초과 시 `CHUNK_MAX_CHARS` hard-split
- `table`: HTML/동등 표 전체를 1 chunk (불가피한 대형만 hard-split)
- `image`: caption 텍스트로 청크, reading order 유지
- heading block은 chunk text에 넣지 않고 후속 block `heading_path`에만 반영
- 계층형 트리 청킹은 별도 DESIGN 선확정 없이 범위 제외
...
### Unified Knowledge Search (3.4)

MS GraphRAG Query Engine을 **retrieval-only**로 매핑한다. 답변 합성은 agents-runtime 에이전트가 담당.

| `mode` | 채널 | 구현 |
|--------|------|------|
| `basic` | chunks FTS + pgvector RRF | `retrieval/modes/basic.py` → `hybrid_search` |
| `global` | community wiki (`wiki_pages`) FTS + vector RRF | `retrieval/modes/global_.py` |
| `local` | entity mirror + Nebula 1-hop + chunk 역참조 | `retrieval/modes/local.py` |
| `drift` | global primer + local/basic follow-up RRF | `retrieval/modes/drift.py` |
| `auto` | 휴리스틱 라우터 | `retrieval/router.py` |

| 모듈 | 역할 |
|------|------|
| `path_graph.retrieval.unified` | `knowledge_search()` — 단일 진입점 |
| `path_graph.retrieval.contracts` | `SearchRequest` / `SearchHit` / `SearchResponse` |
| `path_graph.meta.pg` | `wiki_pages`·`entities` 검색 인덱스 |
| `path_graph.retrieval.indexing` | wiki ingest·graph upsert 시 인덱스 동기화 |

Admin API: `api_search_project(..., mode=, include_graph=, sub_queries=)`. 응답 `{hits[], mode_resolved, graph_context?, results[]}` (`results`는 `hits` alias).
...
| 진입점 | 용도 |
|--------|------|
| `python -m path_graph.steps.retrieval_search` | CLI — `--tenant` `--project-id` `--query` `--mode` `--include-graph` `--json` |
| `path_graph.admin.retrieval.api_search_project` | domain API — BFF가 `asyncio.to_thread`로 호출 |
| agents-runtime `GET /api/pipeline/projects/{id}/search` | Admin Console — `q`, `top_k`, `mode`, `include_graph` |

응답: `{query, project_id, project_slug, mode_resolved, hits[], results[]}` — `hits`는 통합 스키마(`kind`, `id`, `text`, `provenance`).
...
| 포맷 | 이미지 / 의존 | limits (memory) |
|---|---|---|
| hwp/hwpx | `rhwp-batch` v0.7.15 (GH release `x86_64-unknown-linux-gnu` tarball) | 2Gi |
| pdf | PyMuPDF + pymupdf4llm (+ VL OCR 시 sglang HTTP) | 2Gi (OCR 시 4Gi 검토) |
| docx/pptx/xlsx/xls | `unstructured[docx,pptx,xlsx]` (LibreOffice 없음) | 2Gi |
| txt/md | 경량 텍스트 → blocks | 2Gi |
| legacy `.doc` / `.ppt` | 미지원 — ingest `dead_letter` | — |
...
## VL OCR ingest — 스캔 PDF / image-heavy fallback

**상태**: 스캔 PDF VL OCR은 구현됨. native blocks 전환(#260) 시 1차 파서를 markitdown이 아닌 **PyMuPDF router**로 재정렬한다. 계약 변경 시 [ARCHITECTURE.md](../ARCHITECTURE.md) §1 Parse·blocks를 따른다.

### 배경·목표

- **문제**: 텍스트 레이어가 없는 **이미지 스캔 PDF**에서 blocks/chunk 0 → `ingest_state`가 `pending`에 머무르면 Console GraphRAG가 실패한다.
- **설계 방향**: **동일 ingest pass 안의 fallback** — PyMuPDF router가 저텍스트·image-heavy로 판정하면 VL OCR → blocks → chunk, 그래도 실패하면 `dead_letter`. OCR은 클러스터 **sglang + Gemma 4 12B** Vision API만 사용(in-process OCR 패키지 없음).
- **범위**: parse 단계. chunk · RAG embed · GraphRAG 이후는 기존 경로 재사용.
- **비범위**: DOCX/PPTX embedded image OCR(후속), raw **0 byte** 파일, markitdown-ocr 도입.

### 트리거 — dead letter fallback vs 동일 pass fallback

| 방식 | 설명 | 채택 |
|------|------|------|
| **동일 pass fallback** (권장) | `ingest_item` 한 번: PDF router → (scan) → VL OCR → blocks/chunk/RAG | **예** |
| dead_letter → Reingest만 | 1차 ingest 실패 후 운영자 UI Reingest | 보조 |
| dead_letter → 자동 2차 WF | `stage: parse_empty` 전용 Argo | **아니오** |

**「0 byte 문서」 구분**

| 케이스 | raw | 1차 parse | fallback |
|--------|-----|-----------|----------|
| 스캔 PDF | bytes > 0 | 저텍스트 + image-heavy, chunk 0 | VL OCR |
| 진짜 빈 파일 | 0 byte | (업로드 거부 권장) | 불가 → `dead_letter` `stage: empty_raw` |
| 손상 PDF | bytes > 0 | `ParseError` | VL OCR **시도 가능**(렌더 성공 시); 아니면 `dead_letter` `stage: parse` |
| 디지털 PDF | bytes > 0 | PyMuPDF4LLM blocks 충분 | fallback **스킵** |

chunk 0이면 `ingest_item`이 성공(True)으로 끝나지 않는다 — fallback 후에도 chunk 0이면 `record_dead_letter(..., stage: ocr_empty)` 및 `return False`.
...
ingest_item
  ...
  → scan/low-text AND OCR_LLM_BASE_URL configured:
       vl_ocr_pdf
         ...
         3. OCR 결과 → blocks → chunk_from_blocks
  → if chunk_count == 0:
...
| `OCR_FORCE` | `false` | PDF native parse 생략·VL OCR만 (디버그) |
| `OCR_LLM_MODEL` | — | `/v1/models` id |
| `OCR_LLM_API_KEY` | `EMPTY` | |
| `OCR_LLM_TIMEOUT_S` | `120` | 페이지당 |
| `OCR_RENDER_DPI` | `200` | PNG 렌더 |
| `OCR_MIN_TEXT_CHARS` | `32` | (선택) router 조기 VL 판정; 기본은 **chunk_count==0** 또는 image_ratio |
