...
```
collect / ingest_web
  → blob store: raw/{tenant}/{source_id}/{content_hash}/{filename}
  ...
  → PG: documents, chunks (meta/pg.py)
  → [optional --rag] rag_index: embed (rag/embed.py) → Qdrant (rag/qdrant_store.py)
```
...
| `PATH_GRAPH_PROJECTS_PER_TENANT` | `4` | Qdrant collection / Nebula Space shard 수 |
...
src/path_graph/
  ...
  chunkers/chunk.py
  contracts/
    schemas.py, s3_keys.py, community.py
  storage/blob.py
  meta/pg.py
